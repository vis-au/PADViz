...
class HeatMap extends Component {
    ...
    renderD3 () {
        ...
        d3.json("json/heatmap?type").then(function(data){
            // svg
            // .attr("width", graphWidth + margin.left + margin.right)
            // .attr("height", height + margin.top + margin.bottom)
            // .append("g")
            ...
            const lPatchWidth = 600;
            const legends=svg.append("g").attr("class","legends")
            .attr("transform","translate("+((graphWidth+margin.right)/2-lPatchWidth/2 -margin.left/2)+","+(height+margin.bottom-35-20)+")");
            
            legends.append("g").attr("class","LegRect")
                .attr("transform","translate(0,"+15+")")
                .selectAll("rect").data(scale.range()).enter()
                .append("rect").attr("width",lPatchWidth/scale.range().length+"px").attr("height","20px").attr("fill",function(d){ return d})
                .attr("x",function(d,i){ return i*(lPatchWidth/scale.range().length) });
